...
def works_kb(topics: list, page: int = 0, total_pages: int = 1) -> InlineKeyboardMarkup:
    ...
    nav = []
    nav.append(InlineKeyboardButton(text="⬅", callback_data=pack("works", "page", max(page - 1, 0))))
    nav.append(InlineKeyboardButton(text=f"{page + 1} / {max(total_pages, 1)}", callback_data=pack("works", "page", page)))
    nav.append(InlineKeyboardButton(text="➡", callback_data=pack("works", "page", min(page + 1, max(total_pages - 1, 0)))))
    b.row(*nav)
    b.button(text="🏠 В меню", callback_data=pack("nav", "menu", 0))
    b.adjust(1)
    return b.as_markup()
